...
from modules.display import (
    select_subsystem,
    display_subtopics,
    select_subsystem_topic,
    get_url_from_subsystem,
    select_component,
)
...
from modules.gui import (
    print_styled_items,
    print_styled_component,
    print_styled_items_with_keyword_highlights,
    print_styled_component,
)
import json
...
def scan_subsystem_items():
    ...
    val = console.input(
        "[cyan]Press Enter to Continue or (S to Search Keyword)...[/cyan]"
    )
...
while True:
    menu_lines = "\n".join(
        [f"[cyan]{item[0]}[/cyan] [bold]{item[1]}[/bold]" for item in menu_items]
    )
    ...
    try:
        ...
        if choice == "1":
            ...
            console.print(
                "[bold red]Invalid option. Please choose a number between 1 and 5.[/bold red]"
            )
